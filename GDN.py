...
class Agent:
    def __init__(self,
        ...
        self.func_enemy_obs = GAT(nfeat = feature_size,
                                  nhid = hidden_size_obs,
                                  nheads = n_multi_head,
                                  nclass = n_representation_obs,
                                  dropout = dropout,
                                  alpha = 0.2 ).to(device)
        self.func_ally_comm = GAT(nfeat = feature_size+n_representation_obs,
        ...
        self.optimizer = optim.RMSprop(self.eval_params, lr=3e-4)
    ...
    def get_node_representation(self, node_feature, edge_index_enemy, edge_index_ally, n_node_features, mini_batch = False):
        if mini_batch == False:
            with torch.no_grad():
                ...
                h_enemy_obs = self.func_enemy_obs(node_feature, edge_index_enemy, n_node_features, mini_batch = mini_batch)
                edge_index_ally = torch.tensor(edge_index_ally, dtype=torch.long, device=device)
                cat_feature = torch.concat([node_feature, h_enemy_obs], dim=1)
            ...
            cat_feature = torch.concat([node_feature, h_enemy_obs], dim=2)
            node_representation = self.func_ally_comm(cat_feature, edge_index_ally, n_node_features, mini_batch = mini_batch)
        return node_representation

    def train(self):
        node_features, action_features, edge_indices_enemy, edge_indices_ally, rewards, dones, node_features_next, edge_indices_enemy_next, edge_indices_ally_next, avail_actions_next = agent.buffer.sample()
        n_node_features = torch.tensor(node_features).shape[1]
        obs = self.get_node_representation(node_features,
                                           edge_indices_enemy,
                                           edge_indices_ally,
                                           n_node_features,
                                           mini_batch=True)

        q = [self.cal_Q(obs, action_features, agent_id) for agent_id in range(self.num_agent)]
        q_tar = [self.cal_Q(obs, action_features,agent_id) for agent_id in range(self.num_agent)]
    ...
    def cal_Q(self, obs, actions, action_features, avail_actions_next, agent_id, target = False):
        if target == False:
            ...
            obs_and_action = torch.concat([obs_n, action_features], dim=2)
            ...
            with torch.no_grad():
                ...
                obs_and_action_next = torch.concat([obs_next, action_features_next], dim=2)
    ...
    def sample_action(self, node_representation, action_feature, avail_action, epsilon):
        ...
        action_feature = torch.tensor(action_feature, device=device, dtype = torch.float64)
