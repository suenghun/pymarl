...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        self.num_total_unit_types = num_total_unit_types *2
    ...
    def reset(self):
        ...
        """
        self.n_node_features = 4 + 6 + 6 + self.num_total_unit_types
        self._episode_steps = 0
        ...
        self.unit_type_ids = list(set(unit_types))
    ...
    def get_action_feature(self):
        ...
        action_feature = np.concatenate([no_attack_action_feature, attack_action_feature], axis=0)                                    # concat dim                     : num_enemy+n_actions_no_attack, n_actions_no_attack + feature_dim
        action_feature = action_feature.tolist()
        return action_feature

    def get_enemy_feature(self):
        enemy_feature = np.array([self.get_node_feature(node_id) for node_id in range(self.n_agents, self.n_agents + self.n_enemies)]) # shape : num_enemy, feature_dim
    ...
    def get_ally_visibility_edge_index(self):
...
#                         edge_index[0].append(e_id+self.n_agents)
    ...
    def get_node_feature(self, node_id):
        ...
        if unit.health > 0:
            ...
            if unithistoricinfo.last_pos_x == None:
                ...
                health_1st_moment = -(unit.health - unithistoricinfo.last_health) / (self._step_mul*self.health_scaling_factor)
                shield_1st_moment = -(unit.shield - unithistoricinfo.last_shield) / (self._step_mul*self.health_scaling_factor)
                ...
                health_2nd_moment = -(health_1st_moment - unithistoricinfo.last_health_1st_moment) / (self._step_mul*self.health_scaling_factor)
                shield_2nd_moment = -(shield_1st_moment - unithistoricinfo.last_shield_1st_moment) / (self._step_mul*self.health_scaling_factor)
        ...
        unit_type_feats[self.unit_type_ids.index(unit.unit_type)] = 1

        last_action = unithistoricinfo.last_action_node


        node_feature = np.concatenate((move_avail_feats, pos_feats, unit_type_feats, health_and_shield_feats))
    ...
    def get_env_info(self):
        ...
        env_info["node_features"] = 4 + 6 + 6 + self.num_total_unit_types
